import { computePositionSize, getCommissionPerSide } from "../utils/calculations.js";
...
export default function PositionCalculator({
  ...
  const { pointValue, riskDollars, slPoints, tpPoints, lotSize, lossAmt, winAmt, rrr, commission, lossAmtNet, winAmtNet } =
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto", padding: "28px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12, marginBottom: 16 }}>
        ...
        <Card label="Commission (RT)" value={fmtDollar(commission)} color="#a78bfa" sub={commission ? `${fmtDollar(getCommissionPerSide(contract))}/side × ${fmt(lotSize)} lots × 2` : null} />
        <Card label="Max Loss (net)" value={fmtDollar(lossAmtNet)} color="#ff4d6d" sub={lossAmt ? `${fmtDollar(lossAmt)} + ${fmtDollar(commission)} fees` : null} />
        <Card label="Potential Win (net)" value={fmtDollar(winAmtNet)} color="#00e5a0" sub={winAmt ? `${fmtDollar(winAmt)} − ${fmtDollar(commission)} fees` : null} />
        <Card label="Net RRR" value={winAmtNet && lossAmtNet ? (winAmtNet / lossAmtNet).toFixed(2) : "--"} color="#f5c842" sub={winAmtNet && lossAmtNet ? "after commission both sides" : null} />
      </div>
