...
export default function GCJournal() {
  ...
  const [calendarDayFilter, setCalendarDayFilter] = useState(null); // "YYYY-MM-DD" or null
  ...
  const saveBulkEdit = async () => {
    ...
    try {
      ...
      for (const id of selectedIds) {
        ...
        // Merge — only overwrite fields that have a value in bulkForm
        const merged = { ...existing, ...updates };
        // Recalc RRR/points if SL or TP changed
      ...
      setBulkForm({ tradeType: "", direction: "", htfBias: "", marketStructure: "", candlePattern: "", wickDirection: "", session: "", tradeMode: "", grade: "", executionGrade: "", stopLoss: "", takeProfit: "", news: "", newsImpact: "", notes: "" });
        ...
        <div style={{ maxWidth: 1300, margin: "0 auto", padding: "28px 20px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {selectedIds.size > 0 && (
              <>
                ...
                <button onClick={() => setSelectedIds(new Set())} style={{ padding: "6px 10px", borderRadius: 7, border: "1px solid #2a2f3a", background: "transparent", color: "#6b7280", fontSize: 10, cursor: "pointer", fontFamily: "inherit" }}>Clear</button>
              </>
            )}
            {trades.length > 0 && <button onClick={deleteAllTrades} style={{ marginLeft: selectedIds.size > 0 ? 0 : "auto", padding: "6px 14px", borderRadius: 7, border: "1px solid #ff4d6d55", background: "rgba(255,77,109,0.07)", color: "#ff4d6d", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Delete All</button>}
          </div>
          ...
          {bulkEditOpen && selectedIds.size > 0 && (
            <div style={{ background: "#0d1117", border: "1px solid #f5c84233", borderRadius: 12, padding: 20, marginBottom: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#f5c842", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Bulk Edit — {selectedIds.size} trades</div>
              <div style={{ fontSize: 10, color: "#4b5563", marginBottom: 14 }}>Only filled fields will be applied. Empty fields are left unchanged on each trade.</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(175px, 1fr))", gap: 12, marginBottom: 16 }}>
                {[
                  ...
                  <div key={key}>
                    ...
                    {type === "select" ? (
                      <select value={bulkForm[key]} onChange={e => setBulkForm(f => ({ ...f, [key]: e.target.value }))}
                        style={{ ...inp, color: bulkForm[key] ? "#f5c842" : "#4b5563", border: bulkForm[key] ? "1px solid #f5c84244" : "1px solid #2a2f3a" }}>
                        <option value="">— leave unchanged —</option>
                        {opts.map(o => <option key={o} value={o}>{o}</option>)}
                      </select>
                    ) : (
                      <input type="number" step="0.1" value={bulkForm[key]} onChange={e => setBulkForm(f => ({ ...f, [key]: e.target.value }))} placeholder="leave blank to skip"
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input value={bulkForm.notes} onChange={e => setBulkForm(f => ({ ...f, notes: e.target.value }))} placeholder="Text appended to each trade's notes" style={inp} />
              </div>
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <button onClick={saveBulkEdit} disabled={syncing} style={{ padding: "10px 24px", borderRadius: 9, border: "none", background: syncing ? "#2a2f3a" : "linear-gradient(135deg,#f5c842,#ff9a3c)", color: syncing ? "#6b7280" : "#070b12", fontWeight: 700, fontSize: 11, cursor: syncing ? "not-allowed" : "pointer", fontFamily: "inherit", letterSpacing: 2 }}>
                  {syncing ? "Saving..." : `Apply to ${selectedIds.size} Trade${selectedIds.size !== 1 ? "s" : ""}`}
                </button>
                <button onClick={() => { setBulkEditOpen(false); setBulkForm({ tradeType: "", direction: "", htfBias: "", marketStructure: "", candlePattern: "", wickDirection: "", session: "", tradeMode: "", grade: "", executionGrade: "", stopLoss: "", takeProfit: "", news: "", newsImpact: "", notes: "" }); }} style={{ padding: "10px 18px", borderRadius: 9, border: "1px solid #2a2f3a", background: "transparent", color: "#6b7280", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                  Cancel
                </button>
                {syncError && <span style={{ fontSize: 11, color: "#ff4d6d" }}>{syncError}</span>}
              </div>
                ...
                return (
                  <div key={date} style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "8px 12px", background: "#0d1117", borderRadius: "10px 10px 0 0", border: "1px solid #1f2937", borderBottom: "none" }}>
                      ...
                      <div onClick={() => {
                        ...
                        setSelectedIds(prev => {
                          ...
                          if (dayAllSelected) ids.forEach(id => next.delete(id));
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                      {dayTrades.map(t => (
                        <div key={t.id} style={{ background: selectedIds.has(t.id) ? "rgba(245,200,66,0.03)" : "#0d1117", border: `1px solid ${selectedIds.has(t.id) ? "#f5c84233" : "#1f2937"}`, borderTop: "none", overflow: "hidden" }}>
                          <div style={{ padding: "11px 16px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                            {/* Checkbox */}
                            <div onClick={e => { e.stopPropagation(); setSelectedIds(prev => { const next = new Set(prev); next.has(t.id) ? next.delete(t.id) : next.add(t.id); return next; }); }}
                              style={{ width: 16, height: 16, borderRadius: 4, border: `2px solid ${selectedIds.has(t.id) ? "#f5c842" : "#2a2f3a"}`, background: selectedIds.has(t.id) ? "rgba(245,200,66,0.2)" : "transparent", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                              {selectedIds.has(t.id) && <span style={{ fontSize: 9, color: "#f5c842", fontWeight: 900 }}>✓</span>}
                            </div>
                            <div onClick={() => handleExpand(t.id)} style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, cursor: "pointer", flexWrap: "wrap" }}>
      ...
      {!loading && view === "analytics" && (
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "28px 20px" }}>
          ...
          {(() => {
            ...
            // Build daily P&L map from ALL trades (not filtered)
            const dayMap = {};
            trades.forEach(t => {
              if (!t.entryDatetime) return;
              const d = t.entryDatetime.slice(0, 10);
              if (!dayMap[d]) dayMap[d] = { pnl: 0, trades: 0, wins: 0 };
              dayMap[d].pnl    += (parseFloat(t.points) || 0) * 10; // pts → dollars (MGC)
              dayMap[d].trades += 1;
              if (t.outcome === "Win") dayMap[d].wins += 1;
            });

            const prevMonth = () => setCalendarDate(({ year: y, month: m }) =>
              m === 0 ? { year: y - 1, month: 11 } : { year: y, month: m - 1 }
            );
            const nextMonth = () => setCalendarDate(({ year: y, month: m }) =>
              m === 11 ? { year: y + 1, month: 0 } : { year: y, month: m + 1 }
            );
            const today = new Date().toISOString().slice(0, 10);

            // Monthly totals for this calendar view
            ...
            return (
              <div style={{ background: "#0d1117", border: "1px solid #1f2937", borderRadius: 14, padding: 20, marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <button onClick={prevMonth} style={{ width: 32, height: 32, borderRadius: 7, border: "1px solid #2a2f3a", background: "transparent", color: "#e6edf3", fontSize: 16, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center" }}>‹</button>
                  ...
                  <button onClick={nextMonth} style={{ width: 32, height: 32, borderRadius: 7, border: "1px solid #2a2f3a", background: "transparent", color: "#e6edf3", fontSize: 16, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center" }}>›</button>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 3 }}>
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    const isSelected = calendarDayFilter === dateStr;
                    ...
                    return (
                      <div
                        key={day}
                        onClick={() => {
                          setCalendarDayFilter(isSelected ? null : dateStr);
                          if (!isSelected && data) {
                            // Filter analytics month to this month
                        ...
                        {/* Day number */}
                        <div style={{ fontSize: 11, fontWeight: isToday ? 700 : 400, color: isToday ? "#3b82f6" : "#6b7280", marginBottom: 2 }}>{day}</div>
                        {data && (<>
                          {/* P&L */}
                        ...
                        {isToday && <div style={{ position: "absolute", top: 4, right: 4, width: 5, height: 5, borderRadius: "50%", background: "#3b82f6" }} />}
                      </div>
                    );
                  })}
                </div>
                ...
                <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  {calendarDayFilter && (
                    <button onClick={() => { setCalendarDayFilter(null); setAnalyticsMonth("All"); }}
